...
function initTimer () {
	...
	var format = 'dHMS',
		...
		date = new Date(now.getFullYear(), now.getMonth(),  
			now.getDate() + (day == 0 ? 0 : a[day]),
			23, 59, 59
		);
	//if ((date.getTime() - now.getTime()) / 3600000 > 24) format = 'dHM';

	$('.js-timer-1').countdown({
		until: date,
		padZeroes: true,
		format: format
	})
...
function initVideo () {
	$('video').each(function () {
		var oVid = this;
			eBtn = $(oVid).siblings('.video-play-btn');
		$(oVid).add(eBtn).click(function () {
			if (oVid.paused) {
				oVid.play();
				eBtn.hide();
			} else {
...
function colorList () {
	$('.js-select-color-wrap').each(function () {
		var eWrap = $(this);
		eWrap.find('.goods-colors-item').click(function () {
			var eImg = eWrap.find('img');
			eWrap.find('.goods-colors-item').removeClass('active');
			$(this).addClass('active');
			var iLeft = (eImg.parent().width() - eImg.width()) / 2;
			eImg.after($('<img>', {
				src: $(this).data('src'),
				alt: ' ',
				style: 'opacity:0; position:absolute; top:0; left:'+iLeft+'px;'
